...
public class SwerveOptimizer {
   ...
   * appropriate scope for CTRE onboard control (Also is needed for REV).
   *
  ...
  public static BetterSwerveModuleState optimize(BetterSwerveModuleState desiredState,
      Rotation2d currentAngle, double secondOrderOffsetDegrees) {
    ...
    return new BetterSwerveModuleState(targetSpeed, Rotation2d.fromDegrees(targetAngle),
  ...
  public static void antiJitter(BetterSwerveModuleState moduleState,
      BetterSwerveModuleState lastModuleState) {
    if (Math.abs(moduleState.speedMetersPerSecond) <= (6.8 * 0.01)) {
