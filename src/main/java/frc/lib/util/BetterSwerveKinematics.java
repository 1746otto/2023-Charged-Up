 ...
 * </p>
 *
 * Makes use of {@link BetterSwerveModuleState} to add the angular velocity that is required of the
 * module as an output.
 */
public class BetterSwerveKinematics extends SwerveDriveKinematics {
   ...
   *
   * <p>
   * This function also supports variable centers of rotation. During normal operations, the center
   * of rotation is usually the same as the physical center of the robot; therefore, the argument is
   * defaulted to that use case. However, if you wish to change the center of rotation for evasive
   * maneuvers, vision alignment, or for any other use case, you can do so.
   *
   * <p>
   * In the case that the desired chassis speeds are zero (i.e. the robot will be stationary), the
   * previously calculated module angle will be maintained.
   *
   ...
   *         {@link #desaturateWheelSpeeds(BetterSwerveModuleState[], double) DesaturateWheelSpeeds}
  ...
  public BetterSwerveModuleState[] toSwerveModuleStates(ChassisSpeeds chassisSpeeds,
    ...
    if (!centerOfRotationMeters.equals(m_prevCoR)) {
      for (int i = 0; i < m_numModules; i++) {
        m_inverseKinematics.setRow(i * 2 + 0, 0, /* Start Data */
            1, 0, -m_modules[i].getY() + centerOfRotationMeters.getY());
        m_inverseKinematics.setRow(i * 2 + 1, 0, /* Start Data */
            0, 1, +m_modules[i].getX() - centerOfRotationMeters.getX());
        bigInverseKinematics.setRow(i * 2 + 0, 0, /* Start Data */
            1, 0, -m_modules[i].getX() + centerOfRotationMeters.getX(),
            -m_modules[i].getY() + centerOfRotationMeters.getY());
        bigInverseKinematics.setRow(i * 2 + 1, 0, /* Start Data */
            0, 1, -m_modules[i].getY() + centerOfRotationMeters.getY(),
    ...
    chassisSpeedsVector.setColumn(0, 0, chassisSpeeds.vxMetersPerSecond,
    ...
    for (int i = 0; i < m_numModules; i++) {
      ...
      // IDK if this needs to be here but I am too scared to remove it.
      moduleAccelerationStatesMatrix.get(i * 2, 0);
   ...
   *
   * <p>
   * Sometimes, after inverse kinematics, the requested speed from one or more modules may be above
   * the max attainable speed for the driving motor on that module. To fix this issue, one can
   ...
   */
  public static void desaturateWheelSpeeds(BetterSwerveModuleState[] moduleStates,
   ...
   */
  public static void desaturateWheelSpeeds(BetterSwerveModuleState[] moduleStates,
      ChassisSpeeds currentChassisSpeed, double attainableMaxModuleSpeedMetersPerSecond,
      double attainableMaxTranslationalSpeedMetersPerSecond,
      double attainableMaxRotationalVelocityRadiansPerSecond) {
    double realMaxSpeed = Collections.max(Arrays.asList(moduleStates)).speedMetersPerSecond;

    if (attainableMaxTranslationalSpeedMetersPerSecond == 0
        || attainableMaxRotationalVelocityRadiansPerSecond == 0 || realMaxSpeed == 0) {
      return;
    }
