...
public class DriveOverChargeStationCommand extends CommandBase {
  ...
  public void execute() {
    ...
    if (stage == 0) {
      ...
      if ((Math.floor(angle * 180 / Math.PI) == 0.0) && Math.floor(velocities[0]) == 0.0)
        stage = 2;
      swerve.drive(new Translation2d(0.4 * direction, 0.0), 0.0, true, false);
    } else if (stage == 3) {
      if (xpos == 0.0)
        xpos = swerve.getPose().getX();
      // Check if the robot has full traveled outside the community zone.
      if (Math.abs(swerve.getPose().getX() - xpos) > .2)
        stage = 4;
      swerve.drive(new Translation2d(1.0 * direction, 0.0), 0.0, true, false);
    }
  }
