...
public class AutomaticResetOdometryCommand extends CommandBase {
    ...
    public AutomaticResetOdometryCommand(IntSupplier aprilTagSupplier, Supplier<Pose2d> poseSupplier, DoubleSupplier latencySupplier, BiConsumer<Pose2d, Double> odometrySetter) {
    ...
    public void execute() {
        if (getNumAprilTags.getAsInt() >= 3)
            if ((poseSupplier.get().getX() < 3) /*blue alliance community zone */ || poseSupplier.get().getX() > 12.5 /* red */)
